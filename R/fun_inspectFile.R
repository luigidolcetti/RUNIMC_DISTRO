...
inspectFile<-function(fn_file=NULL,
                      ...
                      usedChannelPattern = '[0-9]+[A-z]+-.+\\([A-z]+[0-9]+[A-z]+\\)',
                      ...){

  if (is.null(fn_file)) stop(mError('need a file to inspect'),call. = F)
  if (!file.exists(fn_file)) stop(mError('cannot find this file'),call. = F)

  rawMatrix<-data.table::fread(fn_file,sep='\t',header = T,check.names = F,colClasses = 'numeric')
