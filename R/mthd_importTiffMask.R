...
setMethod('importTiffMask',signature = ('IMC_Study'),
          function(x,fileFolder=NULL,orderAgain=NULL,transposeImage=F,flipImage=NULL,layerName='Tiff_imported', ...){
            ...
            if (!dir.exists(fileFolder)) {stop(mError('cannot find specified folder'))}
            if (is.null(orderAgain)) {message(mWarning('an order has not been specified so files are supposed to be in the correct order'))}
            ...
            for (lbl in 1:length(labelToImport)){
              ...
              if (length(uids)!=length(filesToImport)) {stop(mError('the number of files in this folder does not match the number of samples for this study'))}
              ...
              for (i in 1:length(newClassification)){
                ...
                comparisonRst<-raster::compareRaster(newClassification[[i]],
                                                     ...
                                                     stopiffalse = F,
                                                     res = T)
                if (!comparisonRst){stop(mError('raster and masks do not match'))}
