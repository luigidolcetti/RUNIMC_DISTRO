...
alligatorMap<-function (fn_srt,
  ...
  if (is.character(fn_targetArea)){
    if (!any(fn_targetArea == c('predicted_mean','predicted_median','predicted_mode','predicted_max','predicted_middle'))) stop(mError(('fn_targetArea should be a single numeric or "predicted_mean", "predicted_median, "predicted_mode", "predicted_max", "predicted_middle"')))
  }
  ...
  while(switchOff>0){
    ...
    if (!(nrow(polyG)<3)){
      ...
      polyArea<-.area(polyG)
      polyPerimeter<-.perimeter(polyG)
      polyRoundness<-.roundness(polyArea,polyPerimeter)
