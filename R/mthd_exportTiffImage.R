          ...
          function(x,
            ...
            if (!is.null(layerNames)){
              if (!all(layerNames %in% lyrNms)) {
                stop(mError("some of the specified layers do not match the current layers"),call. = F)} else {
                }
            ...
            for (u in uids){
              .exportTiffImage(fn_Raster = out[[u]],
...
.exportTiffImage<-function(fn_Raster = NULL,
  ...
  if (!all(fn_Layers %in% lyr)) stop(mError("not all specified layers belong to this raster"),call. = F)
