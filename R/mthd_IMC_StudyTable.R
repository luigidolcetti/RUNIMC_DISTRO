...
#' @docType methods
#' @aliases StudyTable,IMC_Study-method
#' @rdname StudyTable-methods
setMethod('studyTable',signature = ('IMC_Study'),
          function(x,...){
            return(x$studyTable)})
...
#' @docType methods
#' @aliases st_uids,IMC_Study
#' @rdname StudyTable-methods
setMethod('st_uids',signature = ('IMC_Study'),
          function(x,...){
...
setMethod('st_uids<-',signature = ('IMC_Study'),
          function(x,...){
            stop(mError('not allowed to change uids'),call. = F)})
...
#' @docType methods
#' @aliases st_folders,IMC_Study
#' @rdname StudyTable-methods
setMethod('st_folders',signature = ('IMC_Study'),
          function(x,...){
            uids<-x$studyTable$uid
...
setMethod('st_folders<-',signature = ('IMC_Study'),
          function(x,...){
            stop(mError('not allowed to change IMC_folder'),call. = F)})
...
#' @docType methods
#' @aliases st_files,IMC_Study
#' @rdname StudyTable-methods
setMethod('st_files',signature = ('IMC_Study'),
          function(x,...){
            uids<-x$studyTable$uid
...
setMethod('st_files<-',signature = ('IMC_Study'),
          function(x,...){
            stop(mError('not allowed to change IMC_text_file'),call. = F)})
...
#' @docType methods
#' @aliases samples,IMC_Study
#' @rdname StudyTable-methods
setMethod('st_samples',signature = ('IMC_Study'),
          function(x,...){
            uids<-x$studyTable$uid
...
#' @docType methods
#' @aliases st_samples<-,IMC_Study
#' @rdname StudyTable-methods
setMethod('st_samples<-',signature = ('IMC_Study'),
          function(x,value,...){
            if (!is.character(value)){
              value<-try(as.character(value))
              if (inherits(value,'try-error')) stop (mError('cannot coerce new vector to character'),call. = F)
            }
            if (length(value)!=nrow(x$studyTable)) stop(mError('length of names does not match number of files'))
...
setMethod('st_samples<-',signature = ('IMC_StudyTable'),
          function(x,value,...){
            ...
            if (length(value)!=nrow(x)) stop(mError('length of names does not match number of files'))
            x$sample<-value
            newTimeStmp<-format(Sys.time(),format="%F %T %Z", tz = Sys.timezone())
            attr(x,'mdtnTimeStmp')<-newTimeStmp
            return(x)
...
#' @docType methods
#' @aliases st_replicates,IMC_Study
#' @rdname StudyTable-methods
setMethod('st_replicates',signature = ('IMC_Study'),
          function(x,...){
            uids<-x$studyTable$uid
...
#' @docType methods
#' @aliases st_rois,IMC_Study
#' @rdname StudyTable-methods
setMethod('st_rois',signature = ('IMC_Study'),
          function(x,...){
            uids<-x$studyTable$uid
...
#' @docType methods
#' @aliases st_bioGroups,IMC_Study
#' @rdname StudyTable-methods
setMethod('st_bioGroups',signature = ('IMC_Study'),
          function(x,...){
            uids<-x$studyTable$uid
